...
import { ensureEffect, ensureOde } from "./lib";
...
export class ResourceTracker {
  ...
  wish(effect: Effect, attempt = false) {
  ...
  clipArt(item: Item, attempt = false) {
  ...
  tome(skill: Skill, attempt = false) {
    if (get("tomeSummons") < 3) {
      useSkill(skill);
  ...
  consumeTo(threshold: number, item: Item) {
